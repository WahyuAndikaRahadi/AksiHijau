...
const AdminDashboard = () => {
  ...
  const loadInitialData = useCallback(async () => {
    ...
    try {
      ...
      if (!allEventsRes.ok || !usersRes.ok)
      ...
      setPendingEvents(
        allEventsData.filter((e: Event) => e.status === "PENDING")
      );
  ...
  const deleteItem = useCallback(
    async (type: "events" | "posts", id: number) => {
      if (!token || deletingId) return;

      const confirmation = await Swal.fire({
        title: `Yakin ingin menghapus ${
          type === "events" ? "event" : "post"
        } ini?`,
      ...
      try {
        ...
        Swal.fire(
          "Berhasil!",
          `${type === "events" ? "Event" : "Post"} berhasil dihapus.`,
          "success"
        );
      } catch (err: any) {
        console.error(err);
  ...
  const moderate = useCallback(
    async (type: "events", id: number, status: "ACCEPTED" | "REJECTED") => {
      if (!token) return;
      ...
      try {
        ...
        if (!res.ok)
          throw new Error((await res.json()).error || "Gagal memoderasi");
        ...
        if (hasLoadedAllEvents) {
          setAllEvents((prev) =>
            prev.map((e) => (e.event_id === id ? { ...e, status } : e))
          );
        }
      } catch (err: any) {
        console.error(err);
  ...
  const awardBadge = async (badgeId: number) => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error((await res.json()).error || "Gagal memberikan badge");
  ...
  if (!token)
    return <div className="p-8 text-center">Harap login sebagai admin</div>;
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            <div>
              ...
              <p className="text-gray-600">
                Kelola konten dan pengguna Eco Heroes
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            {[
              ...
              },
              {
                ...
                value: stats.totalEventsPending,
                icon: Clock,
                color: "from-orange-500 to-orange-600",
              },
              {
                label: "Total Events",
                value: hasLoadedAllEvents ? stats.totalEvents : "...",
                icon: Calendar,
              ...
              },
              {
                label: "Total Posts",
                value:
                  allPosts.length || (activeTab === "posts_all" ? 0 : "..."),
                icon: MessageSquare,
            ...
            {(
              [
                ...
                },
                {
                  ...
                  icon: Calendar,
                  label: "All Events",
                  count: hasLoadedAllEvents ? allEvents.length : "...",
                },
                {
                  key: "posts_all",
                  icon: MessageSquare,
        ...
        {(loading && activeTab === "events_pending") || loadingContent ? (
          ...
          <>
            {activeTab === "events_pending" && (
              <Section
                title={`Events Membutuhkan Persetujuan (${pendingEvents.length})`}
              >
                {pendingEvents.length === 0 ? (
                  <EmptyState message="Tidak ada event yang perlu direview" />
                ) : (
                  pendingEvents.map((event) => (
                    <EventCard
                      ...
                      event={event}
                      onModerate={(status) =>
                        moderate("events", event.event_id, status)
                      }
            ...
            {activeTab === "events_all" && (
              <Section title={`Semua Events (${allEvents.length})`}>
                <div className="space-y-4">
                  {allEvents.length === 0 ? (
                    <EmptyState message="Tidak ada event di database" />
                  ) : (
                    allEvents.map((event) => (
                      <EventCard
                        key={event.event_id}
            ...
            {activeTab === "posts_all" && (
              <Section title={`Semua Posts (${allPosts.length})`}>
                <div className="space-y-4">
                  {allPosts.length === 0 ? (
                    <EmptyState message="Tidak ada post di database" />
                  ) : (
                    allPosts.map((post) => (
                      <PostCard
                        key={post.post_id}
                  ...
                  <table className="w-full">
                    <thead className="bg-gray-50">
                      <tr>
                        ...
                        <th className="px-6 py-3 text-left text-sm font-semibold text-gray-900 min-w-[120px]">
                          Action
                        </th>
                      </tr>
                    </thead>
      ...
      <div className="flex flex-col sm:flex-row md:flex-col gap-2 flex-shrink-0 w-full md:w-auto mt-4 md:mt-0">
        {event.status === "PENDING" && (
          <>
            <button
              ...
              disabled={isDeleting}
              className={`flex items-center justify-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 text-sm font-semibold w-full ${
            ...
            <button
              ...
              disabled={isDeleting}
              className={`flex items-center justify-center gap-2 px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 text-sm font-semibold w-full ${
                isDeleting ? "opacity-50 cursor-not-allowed" : ""
              }`}
            >
        ...
        {event.status !== "PENDING" && (
          <button
            onClick={onDelete}
            disabled={isDeleting}
            className={`flex items-center justify-center gap-2 px-4 py-2 bg-gray-200 text-red-600 rounded-lg hover:bg-gray-300 text-sm font-semibold transition-colors flex-shrink-0 w-full md:w-auto ${
              isDeleting ? "opacity-50 cursor-not-allowed" : ""
            }`}
          >
            {isDeleting ? (
  ...
  >
    <div className="flex flex-col sm:flex-row items-start gap-4">
      ...
      <div className="flex-1 w-full mt-2 sm:mt-0">
        ...
        {post.image_url && (
          <img
            src={post.image_url}
            alt="Post"
            className="w-full max-w-md rounded-lg mb-3 object-cover max-h-96"
            onError={(e) => (e.currentTarget.style.display = "none")}
          />
        )}
        <div className="flex gap-4 text-sm text-gray-500">
          <span>
            <Heart className="w-4 h-4 inline-block mr-1" /> {post.like_count}{" "}
            likes
          </span>
          <span>
      ...
      <button
        onClick={onDelete}
        disabled={isDeleting}
        className={`w-full sm:w-auto flex items-center justify-center sm:justify-start gap-2 px-4 py-2 bg-gray-200 text-red-600 rounded-lg hover:bg-gray-300 text-sm font-semibold transition-colors flex-shrink-0 mt-4 sm:mt-0 ${
          isDeleting ? "opacity-50 cursor-not-allowed" : ""
        }`}
      >
        {isDeleting ? (
...
const Modal: React.FC<{ onClose: () => void; children: React.ReactNode }> = ({
  ...
  >
    ...
    >
      ...
      <button
        onClick={onClose}
        className="mt-6 w-full px-6 py-3 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 font-semibold"
      >
        Batal
      </button>
    </motion.div>
